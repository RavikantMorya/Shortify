...
const ShortenItem = ({createdDate,original_url,short_url,clickCount}) => {
  ...
  const subDomain = import.meta.env.VITE_REACT_FRONT_END_URL.replace(
    ...
    const analyticsHandler = (short_url) =>{
        if (!analyticToggle) {
            setSelectedUrl(short_url); 
        }
    ...
    const fetchMyShortUrl = async () => {
        setLoader(true);
        try {
             const { data } = await api.get(`api/urls/analytics/${selectedUrl}?startDate=2024-12-01T00:00:00&endDate=2028-12-31T23:59:59`, {
                        headers: {
        ...
        //if there is any change in selectedUrl fetchMyShortUrl will be called
        useEffect(() => {
        if (selectedUrl) {
            fetchMyShortUrl();
        }
        ...
        <div className={`flex sm:flex-row flex-col  sm:justify-between w-full sm:gap-0 gap-5 py-5 `}>
            <div className="flex-1 sm:space-y-1 max-w-full overflow-x-auto overflow-y-hidden ">
                ...
                <div className="text-slate-900 pb-1 sm:pb-0   flex items-center gap-2 ">     
                    ...
                    to={import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + `${short_url}`}>
                    {subDomain + "/s/" + `${short_url}`}
                    </Link>
            ...
            <div className="flex  flex-1  sm:justify-end items-center gap-4">
                <CopyToClipboard
                    ...
                    text={`${import.meta.env.VITE_REACT_FRONT_END_URL + "/s/" + `${short_url}`}`}
                >
                ...
                <div
                    onClick={() => analyticsHandler(short_url)}
